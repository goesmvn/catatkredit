...
export default function BarangPage() {
  ...
  const filteredItems = items.filter(i => i.nama_barang.toLowerCase().includes(searchTerm.toLowerCase()))
    ...
    <div className="container pb-nav">
      <div className="header" style={{ position: 'sticky', top: 0, zIndex: 10, background: 'var(--bg)', padding: '16px 20px', margin: '-16px -20px 16px -20px' }}>
        ...
        <div style={{ marginTop: '16px', position: 'relative' }}>
          ...
          <input
            type="text" placeholder="Cari nama barang..." className="form-input"
            value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
            style={{ width: '100%', padding: '16px 16px 16px 48px', fontSize: '16px', borderRadius: '16px', border: '2px solid var(--border)', background: 'var(--white)', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}
          />
        </div>
      </div>
            ...
            filteredItems.map(item => (
              <div key={item.id} className="card list-item" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  ...
                  <p style={{ fontSize: '14px', color: 'var(--primary)', fontWeight: 600, marginTop: '4px' }}>{formatRupiah(item.harga_default)}</p>
                </div>
      ...
      <button className="fab" onClick={() => setShowModal(true)} style={{ bottom: 'calc(var(--nav-h) + 20px)' }}>+</button>
      ...
      {showModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'var(--white)', borderRadius: '24px', width: '100%', maxWidth: '400px', padding: '24px', boxShadow: 'var(--shadow-xl)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div>
                ...
                <input type="text" className="form-input" placeholder="Contoh: Beras 5kg"
                  value={newItemName} onChange={(e) => setNewItemName(e.target.value)} />
              </div>
              <div>
                <label className="section-label">Harga Default</label>
                <input type="text" inputMode="numeric" className="form-input" placeholder="Contoh: 75.000"
                  value={newItemPrice ? new Intl.NumberFormat('id-ID').format(parseInt(newItemPrice.replace(/\D/g, ''), 10) || 0) : ''}
